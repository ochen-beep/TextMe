 ...
 *
 * FIX (bug-2): lastActivity was null on first load.
 *   Date.now() - null == Date.now() (~1.7 trillion ms ≈ 56 years),
 *   so elapsed was always >> threshold and messages fired immediately.
 *   Fixed: initialize lastActivity to Date.now() when falsy, so the
 *   inactivity clock starts from NOW, not from the Unix epoch.
 *
 * FIX (bug-3): Added settings.enabled guard so the timer does nothing
 *   when the extension is disabled via the Enable checkbox, even if
 *   the timer wasn't explicitly stopped by settings-ui.js.
 */
...
async function checkAndSend() {
    ...
    // FIX bug-3: bail if extension is disabled entirely
    ...
    // FIX bug-2: if lastActivity is null/falsy (brand new chat, never interacted),
    // initialize it to NOW so the inactivity clock starts from this moment.
    // Without this fix: Date.now() - null == Date.now() (~1.7e12 ms ≈ 56 years)
    // which always exceeds any threshold, causing immediate message spam.
    if (!phoneData.lastActivity) {
        phoneData.lastActivity = Date.now();
        await savePhoneData();
        return; // start the clock, don't send yet
    }
