 ...
 * License: AGPL-3.0
 *
 * FIX (bug-2): lastActivity initialized to Date.now() instead of null.
 * FIX (bug-3): settings.enabled guard in checkAndSend.
 * FIX: startAutonomousTimer() also called from initPhoneUI() so the timer
 *   starts reliably when a character is loaded with autonomousEnabled=true,
 *   not only when the settings toggle is changed.
 */
...
async function checkAndSend() {
    ...
    // Guard: bail if extension is disabled entirely
    ...
    // If lastActivity is null/falsy, initialize and start the clock
