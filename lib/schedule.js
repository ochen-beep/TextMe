 ...
 * FEAT: manualStatus override — avatar click cycles online→idle→dnd→offline→online.
 * FEAT: Block-based schedule editor — time ranges instead of 24 hourly slots.
 *       Converts legacy hourly schedule to blocks automatically.
 * FEAT: setManualStatus() / clearManualStatus() exported.
 */
...
export async function generateSchedule() {
    ...
    // Convert hourly schedule to blocks and save both formats
    const phoneData = getPhoneData();
    if (phoneData) {
        phoneData.schedule       = schedule;
        phoneData.scheduleBlocks = hourlyToBlocks(schedule);
        await savePhoneData();
    }
 ...
 * Shows 7 collapsible day sections with time-range blocks.
 * Each block has: start time, end time, status badge (click to cycle), activity text, delete button.
...
function buildModalHtml(scheduleBlocks) {
    ...
    <div id="textme-sched-modal" style="font-size:13px;">
        ...
        <p style="color:var(--SmartThemeQuoteColor,#888);margin:0 0 12px;font-size:12px">
            Time blocks define when ${getCharName()} is online/offline.<br>
...
function saveFromModal(phoneData) {
    ...
    // Also update legacy hourly format for compatibility
