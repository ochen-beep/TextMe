 ...
 *
 * FIX: 'today' badge no longer overflows the day-name cell.
 *   The name cell is now a flex sub-row with min-width instead of a
 *   hard width:82px span, so the badge sits beside the day name and
 *   the timeline bar still fills the remaining space via flex:1.
 */
...
function buildMainModalHtml(scheduleBlocks) {
    ...
    if (!scheduleBlocks) {
        ...
        for (const day of DAYS) {
            ...
            // FIX: 'today' badge is now a sibling of the day name text inside
            // a flex sub-row, not a child of a hard-width span.
            // min-width:90px + no hard width → the cell grows to fit both
            // the name and the badge; the timeline bar takes the rest via flex:1.
...
function refreshMainModal(phoneData) {
    ...
    for (const day of DAYS) {
        ...
        // Find the timeline bar div (second child after the name cell)
