...
function buildTimelineBar(blocks) {
    const TOTAL = 24 * 60;
    if (!blocks || blocks.length === 0) return `<div style="flex:1;height:8px;background:${STATUS_COLORS.online};border-radius:4px;"></div>`;
    ...
    const segments = filled.map(b => {
        ...
        return `<div style="flex:${pct};background:${color};height:8px;" title="${title}"></div>`;
    }).join('');
    return `<div style="display:flex;border-radius:4px;overflow:hidden;height:8px;">${segments}</div>`;
}
...
function buildMainModalHtml(scheduleBlocks) {
    ...
    let html = `<div id="textme-sched-main" style="padding:12px;">
        <h3 style="margin-bottom:8px;">📅 Schedule & Status Editor</h3>
        <p style="color:var(--SmartThemeQuoteColor,#888);font-size:12px;margin-bottom:12px;">
            Time blocks define when ${escapeAttr(charName)} is online/offline. Click Edit on any day to modify its schedule.
        </p>`;
    html += `<div style="display:flex;gap:12px;flex-wrap:wrap;margin-bottom:12px;">${Object.entries(STATUS_COLORS).map(([s, c]) =>
        `<span><span style="display:inline-block;width:10px;height:10px;border-radius:50%;background:${c};margin-right:4px;"></span>${STATUS_LABELS[s]}</span>`
    ).join('')}</div>`;
    if (!scheduleBlocks) {
        html += `<p style="color:var(--SmartThemeQuoteColor,#888);">No schedule generated yet.<br>Close this and click "Generate Schedule" first.</p>`;
    } else {
        ...
        for (const day of DAYS) {
            ...
            const todayBadge = isToday ? `<span style="font-size:10px;background:var(--textme-accent,#7c3aed);color:#fff;border-radius:4px;padding:1px 5px;margin-left:6px;">today</span>` : '';
            html += `<div style="display:flex;align-items:center;gap:10px;">
                <span style="width:90px;font-size:13px;font-weight:600;">${dayLabel}${todayBadge}</span>
                <div style="flex:1;">${buildTimelineBar(dayBlocks)}</div>
                <button class="textme-sched-edit-day" data-day="${day}" style="font-size:12px;padding:2px 8px;">Edit</button>
            </div>`;
        }
        html += `</div>`;
    }
...
function refreshMainModal(phoneData) {
    ...
    for (const day of DAYS) {
        ...
        if (barContainer) barContainer.outerHTML = buildTimelineBar(phoneData.scheduleBlocks?.[day] || []);
    }
}
...
function buildDayEditorHtml(day, blocks) {
    const dayLabel = day.charAt(0).toUpperCase() + day.slice(1);
    let html = `<div id="textme-day-editor" style="padding:12px;">
        <h3 style="margin-bottom:8px;">${dayLabel} Schedule</h3>
        <p style="color:var(--SmartThemeQuoteColor,#888);font-size:12px;margin-bottom:10px;">Times are in 24h format (HH:MM). Blocks must not overlap.</p>
        <div class="textme-day-blocks" style="display:flex;flex-direction:column;gap:6px;margin-bottom:10px;">`;
    if (!blocks || blocks.length === 0) {
        html += `<span class="textme-day-empty" style="color:var(--SmartThemeQuoteColor,#888);font-size:12px;">No blocks — character is always Online.</span>`;
    } else {
        blocks.forEach((block, idx) => { html += buildDayBlockRow(day, block, idx); });
    }
    html += `</div><button class="textme-day-add-block" style="font-size:12px;padding:4px 12px;">+ Add Block</button></div>`;
    return html;
}
...
function buildDayBlockRow(day, block, idx) {
    ...
    const color = STATUS_COLORS[b.status] || STATUS_COLORS.online;
    // FIX: escapeAttr() applied to b.from and b.to (time strings from saved JSON).
    // b.activity was already escaped; b.from/b.to now consistently escape too.
    const safeFrom = escapeAttr(b.from);
    const safeTo   = escapeAttr(b.to === '24:00' ? '00:00' : b.to);
    return `<div class="textme-day-block-row" data-idx="${idx}" style="display:flex;align-items:center;gap:6px;">
        <input class="textme-day-from" type="time" value="${safeFrom}" data-idx="${idx}" style="width:90px;">
        <span>–</span>
        <input class="textme-day-to" type="time" value="${safeTo}" data-idx="${idx}" style="width:90px;">
        <button class="textme-day-status-badge" data-idx="${idx}" data-status="${b.status}"
            style="background:${color};color:#fff;border:none;border-radius:6px;padding:2px 8px;cursor:pointer;font-size:12px;">
            ${STATUS_LABELS[b.status] || 'Online'}
        </button>
        <input class="textme-day-activity" type="text" value="${escapeAttr(b.activity || '')}" placeholder="activity" data-idx="${idx}" style="flex:1;">
        <button class="textme-day-del-row" data-idx="${idx}" style="color:#ef4444;background:none;border:none;cursor:pointer;font-size:16px;">×</button>
    </div>`;
}
        ...
        if (del) {
            ...
            if (blocksDiv && blocksDiv.querySelectorAll('.textme-day-block-row').length === 0) {
                if (!blocksDiv.querySelector('.textme-day-empty')) {
                    blocksDiv.insertAdjacentHTML('beforeend', `<span class="textme-day-empty" style="color:var(--SmartThemeQuoteColor,#888);font-size:12px;">No blocks — character is always Online.</span>`);
                }
            }
            return;
        }
...
function saveDay(day, phoneData) {
    ...
    editor.querySelectorAll('.textme-day-block-row').forEach(row => {
        ...
        const activity = row.querySelector('.textme-day-activity')?.value.trim() || '';
...
export function renderScheduleEditor(container) {
    ...
    container.innerHTML = `
        ...
        <div style="font-size:11px;color:var(--SmartThemeQuoteColor,#888);">${blockSummary}</div>
