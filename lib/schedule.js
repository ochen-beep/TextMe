...
export async function setManualStatus(status) {
    ...
    try {
        ...
        if (context?.eventSource?.emit) {
            context.eventSource.emit('textme:statusChanged', {
                prev: prevStatus,
                next: phoneData.manualStatus,
            });
        }
    } catch (e) { /* ignore — event system unavailable */ }
}
    ...
    if (settings.scheduleIncludeWI) {
        try {
            if (typeof context.getWorldInfoPrompt === 'function') {
                // Build a scan buffer respecting wiScanSource (same logic as assembleSystemPrompt).
                const phoneData = getPhoneData();
                ...
                // Merge by timestamp (oldest first), then reverse to newest-first for ST depth scan
                const merged   = [...smsScanEntries, ...rpScanEntries].sort((a, b) => a.ts - b.ts);
                const scanLines = merged.map(e => e.line);

                // Same maxContext logic as assembleSystemPrompt — avoid artificial
                // 4096 cap when the model has a larger context window.
                ...
                // Pre-activate vector WI entries before keyword scan.
