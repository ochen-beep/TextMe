 ...
 *
 * FIX (savePhoneData): removed the redundant post-save block that was
 *   re-assigning chatMetadata[PHONE_KEY] after saveMetadata() completed.
 *   saveMetadata() already persists the full chatMetadata object, so the
 *   re-assignment was a no-op that could obscure future bugs. The function
 *   now simply calls saveMetadata() on the live context — consistent with
 *   the pattern shown in the official ST extension docs.
 */
...
export const DEFAULT_SETTINGS = {
    ...
    smsHistoryDays: 0,     // also limit SMS history by calendar days (0 = disabled)
    ...
    // Conversation gap indicator — injected into <context> when user has been silent
    // Thresholds in minutes (0 = disabled for that level)
    gapThresholds: {
        short:  20,    // minutes → short notice (e.g. 25min ago)
        long:   60,    // minutes → long notice with "mood may have shifted"
        days:   1440,  // minutes (= 24h) → "treat as restarting the conversation"
    },
    // Editable prompt templates for each gap level.
    // Supports {{user}} macro. Set to '' to disable a level without touching thresholds.
    gapPrompts: {
        short: '[Conversation gap: {{user}} last messaged ~{{gap}} ago.]',
        long:  '[Conversation gap: {{user}} last messaged ~{{gap}} ago — the mood or topic may have shifted.]',
        days:  '[Conversation gap: {{user}} last messaged ~{{gap}} ago — treat this as restarting the conversation, not continuing it.]',
    },
    ...
    rpInjectDays:     0,      // also limit by calendar days (0 = disabled)
    ...
    // Contact cosmetics scope
    // 'chat'      — name/avatar stored in chatMetadata (per conversation file)
    // 'character' — name/avatar stored in extensionSettings keyed by character (all chats share it)
    contactScope: 'chat',
    // Per-character overrides: { [charKey]: { name: string, avatar: string } }
    // charKey = char.avatar filename (unique, stable identifier in ST)
    contactOverrides: {},
};
...
/**
 * Slice a messages array applying both a count limit and a days limit.
 * Both limits are optional (0 = disabled). When both are active the result
 * is the intersection — i.e. the more restrictive one wins.
 *
 * @param {Array}  messages   Full phoneData.messages array
 * @param {number} limitCount Max number of messages (0 = all)
 * @param {number} limitDays  Max calendar days back (0 = disabled)
 * @param {function} [getNow] Optional () => Date for testing; defaults to new Date()
 * @returns {Array}
 */
export function sliceMessageHistory(messages, limitCount, limitDays, getNow) {
    if (!messages?.length) return [];

    // Apply count limit first (cheaper)
    let result = limitCount > 0 ? messages.slice(-limitCount) : messages.slice();

    // Apply days limit
    if (limitDays > 0) {
        const now = getNow ? getNow() : new Date();
        // midnight at the start of the oldest allowed day (in game time)
        const cutoff = new Date(now);
        cutoff.setHours(0, 0, 0, 0);
        cutoff.setDate(cutoff.getDate() - (limitDays - 1));
        const cutoffMs = cutoff.getTime();
        result = result.filter(m => !m.time || m.time >= cutoffMs);
    }
...
const DEFAULT_PHONE_DATA = () => ({
    ...
    scenes:          [],
    activeScene:     null,
    // Contact cosmetics — stored per-character chat so each chat/character has its own
    customContactName:   '',   // overrides displayed name in header; '' = use real char name
    customContactAvatar: '',   // URL or data: URI; '' = use ST character avatar
});
 ...
 *
 * Callers are responsible for writing their changes into chatMetadata[PHONE_KEY]
 * BEFORE calling this function (which they all do via getPhoneData() /
 * ensurePhoneData() references). saveMetadata() then persists the whole object.
 *
 * The previous implementation contained a redundant post-save block that
 * re-assigned chatMetadata[PHONE_KEY] = dataToSave after saveMetadata()
 * completed. Because saveMetadata() already serialised the live object,
 * the re-assignment was a no-op that could mask future bugs. It has been
 * removed. The _lastPhoneData cache is still updated by getPhoneData() /
 * ensurePhoneData() on every read, so it remains a valid safety net.
 */
...
/**
 * Shared generation lock used by phone-ui.js (handleSend / regenerate) and
 * autonomous.js (checkAndSend) to prevent both paths from firing API requests
 * simultaneously.
 *
 * phone-ui.js owns writes via setPhoneGenerating().
 * autonomous.js reads via isPhoneGenerating() as an early-exit guard in
 * checkAndSend() — if the user just sent a message and generation is already
 * in flight (including the responseDelay window), the autonomous tick skips
 * the current cycle entirely and waits for the next one.
 *
 * Lifecycle:
 *   setPhoneGenerating(true)  — set at the top of handleSend() / regenerate,
 *                               BEFORE the responseDelay sleep begins.
 *   setPhoneGenerating(false) — set in the finally{} block of handleSend() /
 *                               regenerate, and also in destroyPhoneUI() for
 *                               cleanup on chat switch.
 */
let _phoneGenerating = false;

/** Returns true while phone-ui.js is in the middle of a send/regenerate cycle. */
export function isPhoneGenerating() {
    return _phoneGenerating;
}

/** Set the phone generation lock. Called exclusively by phone-ui.js. */
export function setPhoneGenerating(value) {
    _phoneGenerating = value;
}

// ═══════════════════════════════════════════════
// Contact cosmetics helpers
// ═══════════════════════════════════════════════

/**
 * Stable key identifying the current character across all chats.
 * Uses char.avatar (the filename in /characters/) which is unique and
 * does not change when the character is renamed.
...
/**
 * Read the active contact cosmetics { name, avatar } according to the
 * current contactScope setting:
 *   'chat'      → phoneData.customContactName / customContactAvatar
 *   'character' → extensionSettings.contactOverrides[charKey]
 *
 * Always returns an object; values may be empty strings.
 */
export function getContactData() {
    const settings = getSettings();
    if (settings.contactScope === 'character') {
        const key  = getCharKey();
        const data = key ? (settings.contactOverrides?.[key] ?? {}) : {};
        return { name: data.name || '', avatar: data.avatar || '' };
    }
    // 'chat' scope — stored in phoneData
    const pd = getPhoneData();
    return {
        name:   pd?.customContactName   || '',
        avatar: pd?.customContactAvatar || '',
    };
}

/**
 * Persist contact cosmetics { name, avatar } according to the current scope.
 * Does NOT call save — caller is responsible for saving after this.
 */
export function setContactData(name, avatar) {
    const settings = getSettings();
    if (settings.contactScope === 'character') {
        const key = getCharKey();
        if (!key) return;
        if (!settings.contactOverrides) settings.contactOverrides = {};
        settings.contactOverrides[key] = { name: name || '', avatar: avatar || '' };
    } else {
        const pd = getPhoneData();
        if (!pd) return;
        pd.customContactName   = name   || '';
        pd.customContactAvatar = avatar || '';
    }
}
