...
PROFANITY (МАТ): Use Russian мат with native precision when it fits the character and moment. «Блять» punctuates emotion («ну блять, опять»), «охуеть» = astonishment, «пиздец» = catastrophe or emphasis, «заебись» = genuine approval, «хуёво» = things are bad, «нахуй» = dismissal. Мат is for emotional bonding, raw feeling, or real aggression — never hollow shock value. It's cultural artillery, not decoration.
`,
    summary: `Summarize the recent events from this roleplay chat in 2-3 sentences. Focus on: where characters are, what they're doing, emotional state, recent important events. Write as factual context, not narration.`,
...
Output a JSON object with 7 days (monday through sunday). Each day is an ARRAY OF TIME BLOCKS — not hourly slots. Group the day into natural periods the way a real person lives. Think like a human calendar:
...
export const DEFAULT_SETTINGS = {
    ...
    scheduleIncludeContext: false,
    scheduleIncludeWI: false,

    // Autonomous
    autonomousEnabled: false,
    autonomousThresholds: { online: 5, idle: 15, dnd: 30 },
    maxFollowups: 3,
    ...
    // Response delay by status (seconds)
    responseDelay: {
        online: { min: 0,  max: 3   },
        idle:   { min: 30, max: 120 },
        dnd:    { min: 5,  max: 20  },
...
export function mergeDefaults(saved) {
    ...
    if (!Object.hasOwn(saved, 'smsHistoryDays'))          saved.smsHistoryDays          = 0;
    ...
    if (!Object.hasOwn(saved, 'rpInjectDays'))     saved.rpInjectDays     = 0;
    ...
    // v0.0.3-alpha: merge autonomousTaskPrompt → autonomousPrompt
    if (Object.hasOwn(saved, 'autonomousTaskPrompt')) {
        const task = saved.autonomousTaskPrompt;
        if (task && task.trim()) saved.autonomousPrompt = task;
    ...
    if (saved.promptPresets && typeof saved.promptPresets === 'object') {
        for (const preset of Object.values(saved.promptPresets)) {
            if (preset && typeof preset === 'object' && Object.hasOwn(preset, 'autonomousTaskPrompt')) {
                ...
                if (task && task.trim()) preset.autonomousPrompt = task;
    ...
    const obsoleteKeys = [
        ...
        'temperature', 'inactivityThreshold', 'autonomousTaskPrompt',
    ];
...
const DEFAULT_PHONE_DATA = () => ({
    ...
    autonomousCount: 0,
    autonomousWaitThreshold: null,
    autonomousWaitSince: null,
    autonomousErrorBackoff: null,
    scenes: [],
    activeScene: null,
});
...
export function getPhoneData() {
    ...
    if (!chatMetadata) return ensurePhoneData();
    if (!chatMetadata[PHONE_KEY]) {
        chatMetadata[PHONE_KEY] = _lastPhoneData || DEFAULT_PHONE_DATA();
    }
