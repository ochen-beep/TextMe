 ...
 * current value on demand — chatMetadata is NOT mutated on every tick.
 */
function _startLiveUpdates(tc) {
    ...
    // Tick only to keep module state alive (no DOM or metadata writes per tick)
    realtimeInterval = setInterval(() => {
        // intentionally empty — getCurrentTime() computes live from anchors
    }, 1000);
}
