 ...
 *  - Extension path derived dynamically from import.meta.url (not hardcoded)
 *  - tag changed to 'textme-message'
 *  - soundVolume defaults to 70 (same) but reads from settings if present
 *
 * FIX: extensionPath is now computed from import.meta.url at runtime instead
 * of being hardcoded as 'scripts/extensions/third-party/TextMe'.  This makes
 * the module resilient to the extension being installed under a different folder
 * name and avoids a broken asset URL if ST ever changes its directory layout.
 * The graceful fallback to _generateSynthTone() is preserved — a 404 response
 * is silently caught and the synthesized ping is used instead.
 */
...
/**
 * Resolve the extension root URL from the current module's own URL.
 * import.meta.url points to  …/TextMe/lib/notifications.js
 * One dirname() step gives   …/TextMe/lib/
 * Two dirname() steps give   …/TextMe/            ← extension root
 *
 * The returned string ends WITHOUT a trailing slash so it can be
 * concatenated directly: `${extensionRoot}/assets/notification.mp3`
 */
function getExtensionRoot() {
    try {
        // new URL('../', import.meta.url).href  →  absolute URL ending with /
        // We strip the trailing slash for consistency with the old path format.
...
export async function initNotifications() {
    try {
        ...
        // Try .mp3 first
        try {
            const response = await fetch(`${extensionRoot}/assets/notification.mp3`);
            if (response.ok) {
        ...
        // Try .ogg
        try {
            const response = await fetch(`${extensionRoot}/assets/notification.ogg`);
            if (response.ok) {
                const arrayBuffer = await response.arrayBuffer();
                notificationBuffer = await audioContext.decodeAudioData(arrayBuffer);
