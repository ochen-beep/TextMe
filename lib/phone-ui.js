 ...
 * This ensures that after a chat switch (which rebuilds the DOM via
 * destroyPhoneUI/initPhoneUI) the displayed history matches the new
 * chat's data from context.chat_metadata.
 */
export function togglePhone() {
    ...
    if (phoneOpen) {
        // Always re-render from chatMetadata on open — ensures post-chat-switch
        // correctness and also covers the saveMetadata() object-replacement edge case.
        renderMessages();
        updatePhoneHeader();
        updateStatusBarTime();
        clearBadge();
