 ...
 * FIX: Initial reply delay reduced from 500–1500 ms to 100 ms.
 *      The typing indicator already covers the full API round-trip;
 *      the extra random pause added no useful realism and made the first
 *      bubble feel slow.  Inter-message delays inside streamMessages
 *      (600–2500 ms) are unchanged.
 */
 ...
 * - converts \n → <br> so saved multi-line messages render correctly
 */
...
export function renderMessages() {
    ...
    for (let i = 0; i < phoneData.messages.length; i++) {
        ...
        if (msg.type === 'image') {
            ...
            // FIX: use textToHtml() so \n inside saved messages becomes <br>
            html += `
                <div class="textme-msg textme-msg-${side} ${grouped}" data-idx="${i}">
                    <div class="textme-bubble-wrap">
...
function appendMessage(msg, index) {
    ...
    if (msg.type === 'image') {
        ...
        // FIX: use textToHtml() so freshly-appended messages also respect \n
        div.innerHTML = `
            <div class="textme-bubble-wrap">
                <div class="textme-bubble-content">${textToHtml(msg.text)}</div>
                ${timeHtml}
...
async function handleSend() {
    ...
    try {
        ...
        // FIX: reduced from 500–1500 ms random delay to a fixed 100 ms.
        // The typing indicator already shows throughout the API round-trip;
        // the extra pause just made the first bubble feel slow.
        await sleep(100);
