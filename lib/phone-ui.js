 ...
 * License: AGPL-3.0
 *
 * FIX (bug-1): X button removed — only the back-chevron (‹) minimizes.
 * FIX (bug-2): ensurePhoneData() on init.
 * FIX (bug-3): togglePhone() preserves messages, doesn't blank on re-open.
 * FIX (bug-4): addExternalMessages() uses ensurePhoneData().
 */
 ...
 * FIX: Only re-renders messages if the DOM container is actually empty
 * (e.g. after a fresh initPhoneUI). If messages are already in the DOM
 * we just scroll to bottom — this prevents the blank-on-minimize bug
 * where a re-render could race with in-progress streamMessages().
 */
export function togglePhone() {
    ...
    if (phoneOpen) {
        // Only do a full re-render if the messages container is empty.
        // If it already has children, messages are live — just scroll.
        const container = document.getElementById('textme-messages');
        const hasRendered = container && container.querySelector('.textme-msg');
        if (!hasRendered) {
            renderMessages();
        } else {
            scrollToBottom();
...
export function initPhoneUI() {
    ...
    // Only the back-chevron minimizes — × button removed (was redundant, confusing)
 ...
 * FIX: Uses ensurePhoneData() instead of getPhoneData() so messages
 * are never silently dropped on fresh chats.
 */
export async function addExternalMessages(parts) {
    // FIX: ensure storage exists before writing
    const phoneData = ensurePhoneData();
