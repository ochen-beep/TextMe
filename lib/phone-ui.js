 ...
 *
 * v1.6.0 logging:
 *   — initPhoneUI: logs character name + current SMS message count
 *   — destroyPhoneUI: logs that it is running and cleans up
 *   — streamMessages: logs timing (how long each bubble took)
 *   — addExternalMessages: logs part count and timing
 *   — handleContextAction/regenerate: logs start + completion
 *   — clearPhoneChat: logs number of messages cleared
 */
...
function createPhoneHTML() {
    ...
    <div id="textme-phone" class="textme-phone textme-theme-${theme} textme-scheme-${scheme}" style="display:none;">
        ...
        <div class="textme-input-bar">
            <button id="textme-emoji-btn" class="textme-btn-icon textme-btn-emoji" title="${t('Emoji')}">😊</button>
...
async function streamMessages(messageParts, phoneData) {
    ...
    log.debug(`[streamMessages] Streaming ${messageParts.length} part(s).`);
    ...
    for (let i = 0; i < messageParts.length; i++) {
        ...
        log.debug(`[streamMessages] Part ${i + 1}/${messageParts.length} appended in ${Date.now() - partStart}ms (${finalText.length} chars).`);
    ...
    log.debug(`[streamMessages] All parts done in ${Date.now() - streamStart}ms.`);
}
...
async function handleContextAction(action) {
    ...
    // FIX: hide context menu immediately before any async ops
    hideContextMenu();
    ...
    switch (action) {
        ...
        case 'regenerate':
            if (msg.isUser) break;
            {
                log.info(`[Regenerate] Starting from message index ${idx}.`);
                ...
                if (lastUserMsg) {
                    ...
                    try {
                        ...
                        showTyping();
                        const parts = await generatePhoneResponse(phoneAbortController.signal);
                        log.info(`[Regenerate] Got ${parts.length} part(s) from API.`);
                        hideTyping();
                        await streamMessages(parts, phoneData);
                        log.info('[Regenerate] Complete.');
                    } catch (err) {
                        hideTyping();
                        if (isCancelError(err)) {
...
export async function clearPhoneChat() {
    ...
    const prevCount = phoneData.messages.length;
    phoneData.messages       = [];
    ...
    updateRpInjection();
    renderMessages();
    log.info(`[clearPhoneChat] Cleared ${prevCount} message(s).`);
...
export function initPhoneUI() {
    ...
    log.info(`[initPhoneUI] Initializing for "${getCharName()}" — ${msgCount} SMS message(s) in history.`);
    ...
    document.getElementById('textme-emoji-btn')?.addEventListener('click', (e) => {
        e.stopPropagation();
        const btn = document.getElementById('textme-emoji-btn');
        const input = document.getElementById('textme-input');
    ...
    phoneOpen = false;
    log.info(`[initPhoneUI] Done. Autonomous: ${settings.autonomousEnabled ? 'started' : 'off'}, RP inject: ${settings.rpInjectEnabled ? 'on' : 'off'}.`);
}
...
export function destroyPhoneUI() {
    ...
    if (container) {
        log.info('[destroyPhoneUI] Removing phone container and cleaning up listeners.');
        container.remove();
    }
    ...
    if (phoneAbortController) {
        phoneAbortController.abort();
        phoneAbortController = null;
        log.debug('[destroyPhoneUI] Aborted in-flight generation.');
    }
...
export async function addExternalMessages(parts) {
    log.info(`[addExternalMessages] Received ${parts.length} part(s) from autonomous generation.`);
    ...
    for (let i = 0; i < parts.length; i++) {
        ...
        log.debug(`[addExternalMessages] Part ${i + 1}/${parts.length} appended in ${Date.now() - partStart}ms (${finalText.length} chars).`);
    ...
    log.info(`[addExternalMessages] Done. Total time: ${Date.now() - addStart}ms.`);
