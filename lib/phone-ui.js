...
function createPhoneHTML() {
    ...
    <div id="textme-contact-modal" class="textme-contact-modal" style="display:none;">
        <div class="textme-contact-modal-box">
            ...
            <div class="textme-contact-modal-avatar-preview">
...
async function streamMessages(messageParts, phoneData) {
    for (let i = 0; i < messageParts.length; i++) {
        ...
        // Parse > quote from the model's output — only on the first bubble.
        // If the model quoted something, resolve it to a replyTo reference.
        let finalText = text;
        let replyTo   = null;
        if (i === 0) {
...
function updatePhoneHeader() {
    const settings    = getSettings();
    const displayName = settings.customContactName?.trim() || getCharName();
    const nameEl      = document.querySelector('.textme-header-name');
    if (nameEl) nameEl.textContent = displayName;

    // Avatar: custom overrides ST character avatar
    const customAvatar = settings.customContactAvatar?.trim() || '';
    ...
    if (avatarEl) {
        ...
        if (avatar) {
            ...
            // no avatar at all — show placeholder (don't replace if img already present)
        }
    }
...
/**
 * Refresh the avatar preview inside the contact edit modal.
...
function openContactEditModal() {
    ...
    if (nameInput) nameInput.value = settings.customContactName  || '';
    if (urlInput)  urlInput.value  = settings.customContactAvatar || '';

    // Show current avatar in preview
    _refreshContactModalPreview(settings.customContactAvatar || '');
...
function _onContactUrlInput(e) {
    _refreshContactModalPreview(e.target.value);
}

/** Close the modal and clean up transient listeners. */
...
async function saveContactEdit() {
    ...
    settings.customContactName   = nameInput?.value.trim()  || '';
    settings.customContactAvatar = urlInput?.value.trim()   || '';
...
async function resetContactEdit() {
    const settings = getSettings();
    settings.customContactName   = '';
    settings.customContactAvatar = '';
...
export function initPhoneUI() {
    ...
    document.getElementById('textme-contact-file-input')?.addEventListener('change', (e) => {
        ...
        reader.onload = (ev) => {
            ...
            _refreshContactModalPreview(ev.target.result);
        };
