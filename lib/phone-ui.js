 ...
 *
 * FIX (bug-1): The X button now calls minimizePhone() instead of closePhone().
 *   closePhone() was hiding the bubble permanently. The header X should only
 *   minimize (hide phone window, keep bubble visible). closePhone() is kept
 *   for explicit /phone-close slash command usage only.
 *
 * FIX (bug-2): ensurePhoneData() is called on init so chat_metadata is always
 *   ready before the user can type.
 */
    ...
    <div id="textme-phone" class="textme-phone textme-theme-${theme} textme-scheme-${scheme}" style="display:none;">
        ...
        <div class="textme-header" id="textme-header">
            ...
            <div class="textme-header-actions">
                ...
                <!-- FIX bug-1: title changed to Minimize, handler changed to minimizePhone() -->
                <button class="textme-btn-icon textme-btn-close" title="Minimize">
...
async function handleSend() {
    ...
    // FIX: ensurePhoneData() guarantees chat_metadata is initialized even
    // on freshly created chats that haven't been saved to disk yet.
...
async function handleContextAction(action) {
    ...
    switch (action) {
        ...
        case 'regenerate':
            if (msg.isUser) break;
            {
                // Save the last user message BEFORE splice (indices shift after removal)
                let lastUserMsg = '';
 ...
 * The header X button now calls minimizePhone() instead.
 */
...
export function initPhoneUI() {
    ...
    // FIX bug-2: Pre-initialize chat_metadata so getPhoneData() never returns null
    // when the user sends their first message on a fresh chat.
    ensurePhoneData();
    ...
    // FIX bug-1: X button now calls minimizePhone() — keeps bubble visible.
    // Previously called closePhone() which hid the bubble permanently.
