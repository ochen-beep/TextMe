...
import { getFormattedGameTime } from './custom-time.js';
...
export function renderMessages() {
    ...
    for (let i = 0; i < phoneData.messages.length; i++) {
        ...
        const receiptHtml = readReceiptHtml(phoneData.messages, i, settings);
...
function appendMessage(msg, index) {
    ...
    // Receipt for the new message (if it's a user msg)
    const receiptHtml = readReceiptHtml(phoneData.messages, index, settings);

    // If a char message just arrived, strip receipt from the previous user bubble
    if (!msg.isUser) {
        const prevUserIdx = lastUserMsgIndex(phoneData.messages.slice(0, index));
        if (prevUserIdx >= 0) {
...
function readReceiptHtml(messages, index, settings) {
    ...
    return `<div class="textme-read-receipt textme-receipt-delivered">
        <span class="textme-receipt-icon"><i class="fa-solid fa-check-double"></i></span>
        <span class="textme-receipt-text">Delivered</span>
    </div>`;
}
...
async function handleSilentSend() {
    ...
    const phoneData = ensurePhoneData();
    const userMsg = { isUser: true, text, time: Date.now(), silent: true };
    phoneData.messages.push(userMsg);
    phoneData.lastActivity   = Date.now();
    phoneData.autonomousCount = 0;
    appendMessage(userMsg, phoneData.messages.length - 1);
