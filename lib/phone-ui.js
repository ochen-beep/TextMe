...
export function renderMessages() {
    ...
    for (let i = 0; i < msgs.length; i++) {
        ...
        if (!msg.isUser && !msg.replyTo && msg.text?.startsWith('>')) {
            ...
            log.info(`[renderMessages] orphan-check i=${i} text="${msg.text.substring(0,40)}" parsed.text="${parsed.text.substring(0,40)}" same=${parsed.text === msg.text}`);
        ...
        if (!msg.isUser && !displayReplyTo && pendingOrphanQuote) {
            log.info(`[renderMessages] attaching orphan quote to i=${i} text="${msg.text?.substring(0,40)}"`);
            displayReplyTo     = pendingOrphanQuote;
            pendingOrphanQuote = null;
        } else if (!msg.isUser) {
            if (pendingOrphanQuote) log.info(`[renderMessages] discarding orphan quote at i=${i}`);
            pendingOrphanQuote = null;
        }
