 ...
 *
 * FIX (security): XSS in createPhoneHTML() and updatePhoneHeader().
 *   charName, customName, and avatar were inserted into innerHTML without
 *   escaping. A character name containing <script> or a custom avatar URL
 *   with an unescaped double-quote could inject arbitrary HTML/JS.
 *   All user-controlled strings are now passed through escapeHtml() before
 *   being placed into innerHTML template literals.
 *   - createPhoneHTML(): escapeHtml() applied to displayName and avatar src.
 *   - updatePhoneHeader(): escapeHtml() applied to avatar src in innerHTML.
 *   - _refreshContactModalPreview(): escapeHtml() applied to img src.
 *
 * FIX: Context menu now hides immediately when an action is clicked, before
 *   any async operations begin. Previously hideContextMenu() was placed after
 *   the switch statement, so for 'regenerate' the menu stayed visible for the
 *   entire generation (response delay + API call + streaming = 10–30 s).
 *
 * FIX (T-01, T-02, T-03): Replace native confirm() / prompt() with ST Popup API.
 *   Native browser confirm() and prompt() are blocked or unreliable in many
 *   environments: Chrome 92+ blocks them in cross-origin iframes, Android
 *   WebView (Termux) may suppress them entirely, and they look out-of-place on
 *   all platforms. ST's Popup.show.confirm() / Popup.show.input() are the
 *   documented replacement and work everywhere SillyTavern runs.
 *
 *   Replaced:
 *     - clearPhoneChat()          confirm() → Popup.show.confirm()
 *     - handleContextAction()     confirm() → Popup.show.confirm()  (delete)
 *     - handleContextAction()     prompt()  → Popup.show.input()    (edit)
 */

import { EXTENSION_NAME, getSettings, getPhoneData, ensurePhoneData, savePhoneData, getCharName, getUserName, hasCharacter, updateSetting, getContactData, setContactData, setPhoneGenerating } from './state.js';
...
function createPhoneHTML() {
    ...
    // FIX (XSS): escape all user-controlled strings before inserting into innerHTML.
    // escapeHtml() encodes &, <, >, " — prevents tag injection and attribute
    // break-out. data:, https:, and /characters/ URLs are unaffected because
    // none of those characters require encoding.
    const safeName   = escapeHtml(customName || charName);
    ...
    <div id="textme-phone" class="textme-phone textme-theme-${theme} textme-scheme-${scheme}" style="display:none;">
        ...
        <div class="textme-header" id="textme-header">
            <div class="textme-header-back" title="${t('Minimize')}">
                <i class="fa-solid fa-chevron-left"></i>
            </div>
            <div class="textme-header-avatar" id="textme-header-avatar"
                 title="${t('Click to change status')}" style="cursor:pointer;">
                ${avatar
            ...
            <div class="textme-header-info">
                <div class="textme-header-name-row">
                    <div class="textme-header-name">${safeName}</div>
                </div>
                <div class="textme-header-status" id="textme-header-status">...</div>
            </div>
            <!-- header-actions: edit button always visible, next to trash -->
            <div class="textme-header-actions">
                <button class="textme-btn-edit-contact" title="${t('Edit contact')}" id="textme-btn-edit-contact">
                    <i class="fa-solid fa-pen"></i>
                </button>
                <button class="textme-btn-icon textme-btn-clear" title="${t('Clear chat')}">
        ...
        <div class="textme-typing" id="textme-typing" style="display:none;">
            ...
            <span class="textme-typing-text">${safeName}${t(' is typing...')}</span>
        </div>
        ...
        <div class="textme-input-bar">
            <textarea id="textme-input" class="textme-input" placeholder="${t('Type a message...')}" rows="1"></textarea>
            <button id="textme-send" class="textme-btn-send" title="${t('Send (right-click / long-press = silent send)')}">
    ...
    <!-- Floating Bubble -->
    <div id="textme-bubble" class="textme-bubble" title="${t('Open TextMe')}">
    ...
    <div id="textme-context-menu" class="textme-context-menu" style="display:none;">
        ...
        <div class="textme-ctx-item" data-action="regenerate"><i class="fa-solid fa-rotate"></i> ${t('Regenerate')}</div>
        <div class="textme-ctx-item" data-action="edit"><i class="fa-solid fa-pen"></i> ${t('Edit')}</div>
        <div class="textme-ctx-item textme-ctx-danger" data-action="delete"><i class="fa-solid fa-trash"></i> ${t('Delete')}</div>
    </div>

    <!-- Contact Edit Modal -->
    <div id="textme-contact-modal" class="textme-contact-modal" style="display:none;">
        <div class="textme-contact-modal-box">
            <div class="textme-contact-modal-title">${t('Edit Contact')}</div>

            <!-- Scope toggle -->
            <div class="textme-contact-scope-row">
                <span class="textme-contact-scope-label">${t('Save for')}</span>
                <div class="textme-contact-scope-toggle" id="textme-contact-scope-toggle">
                    <button class="textme-scope-btn" data-scope="chat">${t('This chat')}</button>
                    <button class="textme-scope-btn" data-scope="character">${t('All chats')}</button>
                </div>
            </div>

            <!-- Avatar preview (shown when no crop is active) -->
            ...
            <!-- Crop zone (hidden until an image is loaded for cropping) -->
            <div id="textme-contact-crop-wrap" class="textme-contact-crop-wrap" style="display:none;">
                ...
                <div class="textme-contact-crop-hint">${t('Drag to pan · Scroll to zoom')}</div>
            </div>

            <div class="textme-contact-modal-field">
                <label>${t('Display name')}</label>
                <input id="textme-contact-name-input" type="text" placeholder="${t('Leave empty to use character name')}" />
            </div>
            ...
            <div class="textme-contact-modal-actions">
                ...
                <div class="textme-contact-modal-actions-right">
                    <button id="textme-contact-cancel-btn" class="textme-contact-btn textme-contact-btn-ghost">${t('Cancel')}</button>
                    <button id="textme-contact-save-btn" class="textme-contact-btn textme-contact-btn-primary">${t('Save')}</button>
                </div>
            </div>
        </div>
    </div>
...
function _updateReplyBar() {
    ...
    bar.innerHTML = `
        ...
        <button class="textme-reply-bar-close" id="textme-reply-bar-close" title="${t('Cancel reply')}">✕</button>`;
...
export function renderMessages() {
    ...
    for (let i = 0; i < msgs.length; i++) {
        ...
        const replyBtnHtml = `<button class="textme-reply-btn" data-idx="${i}" title="${t('Reply')}">↩</button>`;
...
function appendMessage(msg, index) {
    ...
    if (!first) {
        ...
        if (prevEl) {
            prevEl.querySelector('.textme-msg-time')?.remove();
        }
    }
    ...
    const quoteHtml = displayReplyTo ? replyQuoteHtml(displayReplyTo, charName, userName) : '';
    const replyBtnHtml = `<button class="textme-reply-btn" data-idx="${index}" title="${t('Reply')}">↩</button>`;
...
async function handleContextAction(action) {
    ...
    // FIX: hide context menu immediately — before any async operations.
    // Previously hideContextMenu() was placed after the switch statement,
    // so for 'regenerate' (which awaits response delay + API + streaming)
    // the menu stayed visible for the entire generation (10–30 s).
    hideContextMenu();
    ...
    switch (action) {
        case 'copy':
            try {
                ...
                toastr.error(t('Failed to copy'));
            }
            break;

        case 'delete': {
            // FIX (T-01): replaced native confirm() with ST Popup.show.confirm().
            // Native confirm() is blocked in WebView-based environments (Android/Termux,
            // some desktop Chrome setups) and has no consistent cross-platform styling.
            const deleteIdx = idx; // snapshot before any async gap
        ...
        case 'edit': {
            // FIX (T-02): replaced native prompt() with ST Popup.show.input().
            // Native prompt() is blocked in the same environments as confirm().
            const { Popup } = SillyTavern.getContext();
            const newText = await Popup.show.input(
                t('Edit message'),
                t('Edit message:'),
                msg.text,
            );
...
function updatePhoneHeader() {
    ...
    // Safe: textContent does not interpret HTML — no escaping needed here.
    ...
    if (avatarEl) {
        ...
        if (avatar) {
            if (img) {
                // Safe: assigning to img.src via JS property — browser handles escaping.
                img.src = avatar;
            } else {
                ...
                // FIX (XSS): avatar inserted into innerHTML — must be escaped to prevent
                // attribute injection (e.g. a URL containing `"`).
                avatarEl.innerHTML = `<img src="${escapeHtml(avatar)}" alt="" />${dotHtml}`;
            }
        }
    ...
    // Safe: textContent — no escaping needed.
...
export async function clearPhoneChat() {
    ...
    // FIX (T-01): replaced native confirm() with ST Popup.show.confirm().
    // Native confirm() is blocked in WebView-based environments (Android/Termux,
    // some desktop Chrome configurations) and provides no styling consistency.
    const { Popup } = SillyTavern.getContext();
    const confirmed = await Popup.show.confirm(
        t('Clear chat'),
        t('Clear all phone messages?'),
    );
    if (!confirmed) return;
...
function _initCropper(src) {
    ...
    img.onerror = () => {
        toastr.error(t('Could not load image'));
    };
    img.src = src;
}
...
function _cropOnTouchStart(e) {
    _cropTouches = Array.from(e.touches);
}
...
function _refreshContactModalPreview(url) {
    ...
    if (src) {
        // FIX (XSS): src may be a user-entered URL — escape before placing in innerHTML.
        previewEl.innerHTML = `<img src="${escapeHtml(src)}" alt="" />`;
    } else {
        previewEl.innerHTML = `<div class="textme-avatar-placeholder"><i class="fa-solid fa-user"></i></div>`;
    }
}
...
function openContactEditModal() {
    ...
    const canvas = document.getElementById('textme-contact-crop-canvas');
    if (canvas) {
        _detachCropListeners(canvas);
        _attachCropListeners(canvas);
    }
    const cropWrap    = document.getElementById('textme-contact-crop-wrap');
    const previewWrap = document.getElementById('textme-contact-preview-wrap');
...
async function saveContactEdit() {
    ...
    if (settings.contactScope === 'character') {
        // Per-character data lives in extensionSettings → save settings
