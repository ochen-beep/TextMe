...
function autoResizeInput() {
    ...
    // Keep overflow hidden to prevent scrollbar flash
...
export function minimizePhone() {
    ...
    // Make sure bubble stays visible
 ...
 * This is different from minimize which keeps the bubble visible.
 */
...
export function togglePhone() {
    ...
    // If bubble was hidden (phone was closed), show it first
    if (bubble && bubble.style.display === 'none') {
...
export function initPhoneUI() {
    ...
    // Minimize button (chevron-left) = minimize (phone hides, bubble stays)
    document.querySelector('.textme-header-back')?.addEventListener('click', minimizePhone);

    // Close button (X) = close (phone hides, bubble hides)
    ...
    // Phone starts closed, bubble visible
    phoneOpen = false;
}
