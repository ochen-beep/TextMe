 ...
 * FIX: togglePhone() preserves messages, doesn't blank on re-open.
 ...
 * Only re-renders messages if the DOM container is actually empty.
 */
...
export function initPhoneUI() {
    ...
    // FIX: Start autonomous timer here so it runs whenever the phone UI is
    // initialized with a character — not only when the toggle is changed.
    // This handles the case where autonomousEnabled was already true before
    // the chat loaded (e.g. settings open at startup).
    const settings = getSettings();
