 ...
 *   - When a Connection Profile is configured (settings.connectionProfileId),
 *     TextMe uses ConnectionManagerRequestService with its own AbortController.
 *     The cancel button WORKS — it aborts only TextMe's request.
 *   - When NO profile is configured, TextMe falls back to generateRaw which
 *     shares ST's global AbortController. The send button shows a non-interactive
 *     spinner ("Generating...") instead of a cancel button, because aborting
 *     would require clicking ST's #mes_stop which kills everything.
 *   - If ST's global Stop aborts our generateRaw request, the error is caught
 *     and shown as a neutral 'Cancelled' info toast.
 */
...
import { generatePhoneResponse, hasCancelIsolation } from './prompt-engine.js';
 ...
 * — but ONLY when a Connection Profile is configured.
 * When using generateRaw, this controller exists but abort() has no effect
 * on the actual HTTP request (generateRaw ignores external signals).
 */
let phoneAbortController = null;

/**
 * Flag set to true when the user clicks the cancel button.
 * Used to suppress message streaming even when the API call
 * itself cannot be cancelled (generateRaw path).
 */
...
async function streamMessages(messageParts, phoneData) {
    for (let i = 0; i < messageParts.length; i++) {
        ...
        if (isCancelled) {
            log.info('Message streaming interrupted by cancel.');
            break;
        }
        ...
        if (isCancelled) {
            hideTyping();
            log.info('Message streaming interrupted by cancel after delay.');
            break;
        }
...
async function handleSend() {
    const canCancel = hasCancelIsolation();

    // While generating, the button acts as Cancel ONLY if we have isolation
    if (isGenerating) {
        if (canCancel && phoneAbortController) {
            phoneAbortController.abort();
            isCancelled = true;
            log.info('TextMe generation cancelled by user.');
        } else {
            // No isolation — set the flag to stop streaming at least
            isCancelled = true;
            log.info('TextMe streaming cancelled by user (API call continues).');
        }
        return;
    }
    ...
    try {
        ...
        if (!isCancelled) {
            ...
            log.info('TextMe generation response received but cancelled — skipping streaming.');
        }

    } catch (err) {
        hideTyping();
        if (isCancelError(err) || isCancelled) {
...
function updateSendButton() {
    ...
    if (isGenerating) {
        const canCancel = hasCancelIsolation();
        if (canCancel) {
            // True cancel is possible — show stop icon
            btn.disabled = false;
            btn.title = 'Cancel TextMe generation';
            btn.innerHTML = '<i class="fa-solid fa-stop"></i>';
        } else {
            // No isolation — show spinner, clicking sets isCancelled flag
            btn.disabled = false;
            btn.title = 'Cancel message streaming';
