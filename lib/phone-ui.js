 ...
 * License: AGPL-3.0
 *
 * FIX: X button removed — only the back-chevron (‹) minimizes.
 * FIX: ensurePhoneData() on init.
 * FIX: togglePhone() always re-renders on open to reflect current chatMetadata.
 * FIX: addExternalMessages() uses ensurePhoneData().
 * FIX: startAutonomousTimer() called from initPhoneUI() for reliable start.
 * FIX: updatePhoneHeader() called immediately after DOM init so status dot
 *      reflects the real schedule status (not the hardcoded 'online' in HTML).
 * FIX: Regenerate now deletes the ENTIRE char-message group after the last
 *      user message, and archives the previous response cycle in msg._prevCycles.
 * FEAT: Avatar click cycles manual status override (online→idle→dnd→offline→schedule).
 */
...
function createPhoneHTML() {
    ...
    // Status dot starts neutral — updatePhoneHeader() called after DOM insert
    return `
...
export function renderMessages() {
    ...
    for (let i = 0; i < phoneData.messages.length; i++) {
        ...
        // Previous cycles indicator (regenerated messages)
        const cyclesHtml = msg._prevCycles?.length
            ? `<span class="textme-cycles-indicator" data-idx="${i}" title="${msg._prevCycles.length} previous version(s) — right-click to regenerate again">↻${msg._prevCycles.length}</span>`
            : '';
...
async function handleSend() {
    ...
    // Block response when character is offline
    try {
...
async function handleContextAction(action) {
    ...
    switch (action) {
        ...
        case 'regenerate':
            if (msg.isUser) break;
            {
                // Find the index of the last user message before idx
                let lastUserIdx = -1;
                ...
                // Collect ALL char messages after the last user message
                // (the entire response group, not just the single clicked bubble)
                const groupStart = lastUserIdx + 1; // first char msg index
                const charGroup  = phoneData.messages.slice(groupStart);
                const prevTexts  = charGroup.filter(m => !m.isUser).map(m => m.text);

                // Remove the entire char-message group
                phoneData.messages.splice(groupStart, phoneData.messages.length - groupStart);

                // Archive previous cycle on the user message that preceded this group
...
/** Minimize = hide phone window, keep bubble visible */
 ...
 * Only used for explicit /phone-close slash command.
 */
 ...
 * Always re-renders messages on open to reflect current chatMetadata.
 */
...
function updatePhoneHeader() {
    ...
    if (avatarEl && avatar) {
        ...
        if (img) {
            ...
            // Replace placeholder with image, keep status dot
...
/**
 * Update the header status dot and text based on current schedule / manual override.
 */
...
export function initPhoneUI() {
    ...
    // FIX: Update header immediately after DOM insertion so status dot
    // reflects the real schedule/manual status instead of HTML hardcoded value
    updatePhoneHeader();
    updateStatusBarTime();
 ...
 * Uses ensurePhoneData() so messages are never silently dropped.
 */
