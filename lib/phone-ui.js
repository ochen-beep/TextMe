 ...
 * Bails immediately on desktop (innerWidth > 500).
 */
...
function createPhoneHTML() {
    ...
    // FIX: use custom avatar if set, otherwise fall back to ST character avatar.
    ...
    if (avatarEl) {
        ...
        if (avatar) {
            if (img) {
                ...
                avatarEl.innerHTML = `${dotHtml}<img src="${avatar}" alt="" />`;
            }
        }
...
/**
 * Load an image src into the cropper.
 */
...
export function initPhoneUI() {
    ...
    // ── Visual Viewport API (mobile keyboard fix) ────────────────────
    // Must be called AFTER the phone element is in the DOM.
    setupVisualViewport();
