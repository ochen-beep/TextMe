 ...
 * Pipeline:
 * 1. generateRaw() with selfie prompt → JSON description
 * 2. Send to Image API endpoint
 * 3. Receive image URL/base64
 * 4. Add to phone messages
 *
...
export async function generateSelfie() {
    ...
    try {
        ...
        // Recent phone messages for mood context
    ...
    const descResult = await generateRaw(messages, {
        maxTokens: 1000,
        temperature: 0.8,
    });
    ...
    log.debug('Image API response:', imageResult);

    // Extract image URL (support OpenAI format and common alternatives)
    let imageUrl = null;
