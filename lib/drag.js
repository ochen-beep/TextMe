 ...
 *
 * FIX (bubble invisible after toggle / Termux):
 *   applyBubbleSavedPos() now uses window.visualViewport.height (actual visible
 *   area) instead of window.innerHeight (includes browser chrome on mobile).
 *   A BUBBLE_BOTTOM_MARGIN of 100 px is subtracted so the bubble always stays
 *   above Android address bar / nav bar, even when env(safe-area-inset-bottom)
 *   is unavailable (SillyTavern does not set viewport-fit=cover).
 *   offsetWidth/offsetHeight are not used during init (they are 0 before first
 *   layout); BUBBLE_SIZE constant is used instead.
 */
...
const BUBBLE_SIZE         = 56;  // px — fixed bubble dimensions (matches CSS)
const BUBBLE_BOTTOM_MARGIN = 100; // px — guaranteed clearance from bottom on ALL platforms
...
export function makeBubbleDraggable(bubble) {
    ...
    // Restore saved position (clamped to current visible viewport)
    applyBubbleSavedPos(bubble);

    // Re-clamp on viewport resize (e.g. browser chrome appears/disappears on mobile)
    if (_bubbleResizeHandler) {
        window.removeEventListener('resize', _bubbleResizeHandler);
    }
    ...
    // Also re-clamp when visualViewport resizes (mobile browser toolbar show/hide)
    if (window.visualViewport) {
        window.visualViewport.addEventListener('resize', _bubbleResizeHandler);
    }
    ...
    function applyPos(x, y) {
        ...
        x = clamp(x, 0, vw - BUBBLE_SIZE);
        y = clamp(y, 0, vh - BUBBLE_SIZE - BUBBLE_BOTTOM_MARGIN);
        bubble.style.right  = 'auto';
...
/**
 * Returns the actual visible viewport width.
 * Uses window.visualViewport when available (mobile browsers).
 */
function getViewportWidth() {
    return (window.visualViewport ? window.visualViewport.width : window.innerWidth) || window.innerWidth;
}

/**
 * Returns the actual visible viewport height.
 * Uses window.visualViewport when available — this excludes the browser's
 * address bar and navigation bar on Android, giving the true usable height.
 * Falls back to window.innerHeight on desktop / unsupported browsers.
 */
function getViewportHeight() {
    return (window.visualViewport ? window.visualViewport.height : window.innerHeight) || window.innerHeight;
}

/**
 * Apply (or re-apply) the saved bubble position, clamping to the current
 * *visible* viewport. Called both on init and on window/visualViewport resize.
 *
 * FIX: uses getViewportHeight() (visualViewport-aware) instead of
 * window.innerHeight so the bubble is never pushed behind the browser's
 * address bar / nav bar on Android (Termux, Firefox, Chrome Mobile).
 *
 * FIX: uses BUBBLE_SIZE constant instead of offsetWidth/offsetHeight, which
 * are 0 before first layout when called synchronously from makeBubbleDraggable.
 *
 * FIX: subtracts BUBBLE_BOTTOM_MARGIN so bubble always has guaranteed clearance
 * from the bottom edge — works even when env(safe-area-inset-bottom) returns 0
 * (SillyTavern does not include viewport-fit=cover in its meta viewport tag).
 *
 * @param {HTMLElement} bubble
 */
function applyBubbleSavedPos(bubble) {
    ...
    if (!saved) {
        // No saved position — let CSS handle placement (bottom/right via stylesheet).
        // Do NOT override right/bottom with 'auto' here; that would break the CSS defaults.
        return;
    }
    ...
    bubble.style.right  = 'auto';
    bubble.style.bottom = 'auto';
    bubble.style.left   = clamp(saved.x, 0, vw - BUBBLE_SIZE) + 'px';
    bubble.style.top    = clamp(saved.y, 0, vh - BUBBLE_SIZE - BUBBLE_BOTTOM_MARGIN) + 'px';
}
