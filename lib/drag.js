 ...
 * License: AGPL-3.0
 *
 * FIX: makeBubbleDraggable() now adds a window 'resize' listener that
 * re-clamps bubble position whenever the viewport shrinks, so the bubble
 * never goes off-screen when the browser window is resized.
 *
...
export function makeBubbleDraggable(bubble) {
    ...
    // FIX: re-clamp on every viewport resize so the bubble stays visible
    // when the browser window is made smaller.
    // Store the handler in module scope so cleanupDragListeners() can remove it.
    if (_bubbleResizeHandler) {
        window.removeEventListener('resize', _bubbleResizeHandler);
    }
