...
export function loadSettingsUI() {
    ...
    $('#textme_enabled').prop('checked', settings.enabled).on('change', function () {
        ...
        if (enabled) {
            ...
            // That check was silently blocking re-init when ST returned undefined for
            // characterId at toggle time (race / timing issue).
            // initPhoneUI() has its own hasCharacter() check and logs a warning if
            // no character is selected — so the outer guard was both redundant and harmful.
