 ...
 *
 * FIX (T-03): Replace native confirm() / prompt() in _initPromptPresets() with
 * ST Popup API. Affected operations:
 *   - #textme_preset_save    — prompt()  → Popup.show.input()
 *   - #textme_preset_delete  — confirm() → Popup.show.confirm()
 *   - #textme_preset_reset_all — confirm() → Popup.show.confirm()
 * Native dialogs are blocked in Android WebView / Termux environments and in
 * certain desktop WebView wrappers. ST's Popup.show.* work everywhere ST runs.
 * All three handlers are now async and use a fresh SillyTavern.getContext()
 * call to avoid stale closure captures.
 */
 ...
 *
 * FIX (T-03): All three confirm/prompt dialogs replaced with ST Popup API.
 * Handlers that use Popup.show.* are made async — jQuery .on('click', async fn)
 * works correctly: jQuery does not await the returned Promise, but that is fine
 * here because the handlers are self-contained and errors are caught internally.
 */
function _initPromptPresets(settings, context) {
    ...
    // FIX (T-03): prompt() → Popup.show.input()
    // Native prompt() is blocked in Android WebView / Termux and in some
    // desktop WebView environments. Popup.show.input() works everywhere ST runs.
    $('#textme_preset_save').on('click', async function () {
        const defaultName = settings.activePreset || '';
        const { Popup } = SillyTavern.getContext();
        const name = await Popup.show.input(
            'Save preset',
            'Save current prompts as preset — enter a name:',
            defaultName,
        );
        if (!name?.trim()) return;
    ...
    // FIX (T-03): confirm() → Popup.show.confirm()
    $('#textme_preset_delete').on('click', async function () {
    ...
    // FIX (T-03): confirm() → Popup.show.confirm()
    $('#textme_preset_reset_all').on('click', async function () {
        const { Popup } = SillyTavern.getContext();
        const confirmed = await Popup.show.confirm(
            'Reset all prompts',
            'Reset ALL prompts to built-in defaults?\nThis will overwrite your current prompt text.',
        );
        if (!confirmed) return;
        for (const key of PRESET_PROMPT_KEYS) {
