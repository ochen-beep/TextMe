 ...
 *
 * FIX (v1.4.1): _initConnectionProfileDropdown() is now guarded by a module-level
 * flag so it runs only once even if loadSettingsUI() is called multiple times.
 * Without the guard, handleDropdown() re-registered its internal 'change' listener
 * on every call, so onChange fired N times in parallel and saveSettingsDebounced()
 * was called N times simultaneously — producing a race that crashed the ST server.
 * The onChange callback now also acquires a fresh context via SillyTavern.getContext()
 * instead of closing over a stale reference, and the whole body is wrapped in
 * try/catch to prevent unhandled promise rejections from escaping.
 *
 ...
 *
 * FIX (v1.0.4): Removed illegal jQuery .slideUp() on ST's internal drawer element.
 * The previous code called:
 *   $('#extensions_settings2').closest('.drawer-content').slideUp(200)
 * This applied inline style="display:none" to #rm_extensions_block while
 * leaving the openDrawer CSS class intact. ST's drawer system is class-based:
 * toggleDrawer() checks hasClass('openDrawer') — after our slideUp() it found
 * openDrawer=true, tried to CLOSE the already-closed drawer, and the button
 * appeared to be permanently broken (visible stuck in a zombie open state).
 * Extensions MUST NOT touch ST's internal drawer DOM with raw jQuery animations.
 *
 * v0.0.3-alpha: Removed autonomousTaskPrompt from PRESET_PROMPT_KEYS and bindPrompt.
 * The two-field split has been collapsed into a single autonomousPrompt.
 * state.js mergeDefaults() handles migration of existing saves and presets.
 *
 * v1.6.0 logging:
 *   — bindNumber now logs value changes with setting key + new value
 *   — Runtime log level toggle button wired to getLogLevel/setLogLevel
 */
...
import { updateRpInjection } from './rp-inject.js';
import { exportLogs, log, getLogLevel, setLogLevel } from './logger.js';
...
export function loadSettingsUI() {
    ...
    function bindNumber(selector, key) {
        $(selector).val(settings[key]).on('change', function () {
            ...
            updateSetting(key, val);
            context.saveSettingsDebounced();
            log.debug(`[Settings] "${key}" changed → ${val}`);
        });
    }
    ...
    //
    // FIX (v1.0.4): The previous implementation called:
    //   $('#extensions_settings2').closest('.drawer-content').slideUp(200)
    // to auto-close the extensions drawer after opening the phone. This was wrong:
    // jQuery .slideUp() injects inline style="display:none" on #rm_extensions_block
    // while leaving the CSS class openDrawer unchanged. ST's drawer toggle logic
    // reads hasClass('openDrawer') to decide direction — so after our .slideUp()
    // it found openDrawer=true, issued another slideUp (close), and the drawer
    // became stuck in a zombie state: hidden but logically "open". Every subsequent
    // click on the Extensions icon tried to close-again with no visible effect.
    //
    // The fix is simply to NOT touch ST's drawer at all. The user can dismiss the
    // settings panel themselves by tapping the Extensions icon once — that calls
    // ST's own toggleDrawer() which correctly syncs both class and inline style.
    $('#textme_show_phone').on('click', function () {
        ...
        openPhone();
        // NOTE: We intentionally do NOT close the ST extensions drawer here.
        // See the comment above for why manipulating ST's internal drawer DOM
        // via jQuery .slideUp() breaks the Extensions button permanently.
    });

    bindNumber('#textme_max_tokens', 'maxTokens');
    bindNumber('#textme_sms_history', 'smsHistory');
    bindNumber('#textme_sms_history_days', 'smsHistoryDays');
    ...
    _initConnectionProfileDropdown();
    ...
    // ── Conversation Gap ──
    function bindGapThreshold(selector, key) {
        $(selector).val(settings.gapThresholds?.[key] ?? DEFAULT_SETTINGS.gapThresholds[key]).on('change', function () {
            if (!settings.gapThresholds) settings.gapThresholds = { ...DEFAULT_SETTINGS.gapThresholds };
            settings.gapThresholds[key] = Number($(this).val());
            context.saveSettingsDebounced();
            log.debug(`[Settings] gapThresholds.${key} → ${settings.gapThresholds[key]}min`);
        });
    }
    function bindGapPrompt(selector, key) {
        $(selector).val(settings.gapPrompts?.[key] ?? DEFAULT_SETTINGS.gapPrompts[key]).on('change', function () {
            if (!settings.gapPrompts) settings.gapPrompts = { ...DEFAULT_SETTINGS.gapPrompts };
            settings.gapPrompts[key] = $(this).val();
            context.saveSettingsDebounced();
        });
    }
    bindGapThreshold('#textme_gap_threshold_short', 'short');
    bindGapThreshold('#textme_gap_threshold_long',  'long');
    bindGapThreshold('#textme_gap_threshold_days',  'days');
    bindGapPrompt('#textme_gap_prompt_short', 'short');
    bindGapPrompt('#textme_gap_prompt_long',  'long');
    bindGapPrompt('#textme_gap_prompt_days',  'days');
    $('#textme_gap_prompts_reset').on('click', function () {
        const def = DEFAULT_SETTINGS.gapThresholds;
        const defP = DEFAULT_SETTINGS.gapPrompts;
        settings.gapThresholds = { ...def };
        settings.gapPrompts    = { ...defP };
        $('#textme_gap_threshold_short').val(def.short);
        $('#textme_gap_threshold_long').val(def.long);
        $('#textme_gap_threshold_days').val(def.days);
        $('#textme_gap_prompt_short').val(defP.short);
        $('#textme_gap_prompt_long').val(defP.long);
        $('#textme_gap_prompt_days').val(defP.days);
        context.saveSettingsDebounced();
        toastr.success('Gap prompts reset to default.');
    });
    ...
    function bindDelayField(selector, status, field) {
        ...
        $(selector).val(cfg[field] ?? 0).on('change', function () {
            ...
            const val = Number($(this).val());
            settings.responseDelay[status][field] = val;
            context.saveSettingsDebounced();
            log.debug(`[Settings] responseDelay.${status}.${field} → ${val}s`);
        });
    }
    ...
    // ── RP Injection ──
    const DEFAULT_RP_HEADER = DEFAULT_SETTINGS.rpInjectHeader;
    $('#textme_rp_inject_enabled').prop('checked', settings.rpInjectEnabled).on('change', function () {
        const enabled = !!$(this).prop('checked');
        updateSetting('rpInjectEnabled', enabled);
        context.saveSettingsDebounced();
        log.info(`[Settings] "RP Inject" toggled → ${enabled ? 'ON ✓' : 'OFF'}`);
        updateRpInjection();
    });
    bindNumber('#textme_rp_inject_messages', 'rpInjectMessages');
    bindNumber('#textme_rp_inject_days',     'rpInjectDays');
    bindNumber('#textme_rp_inject_depth',    'rpInjectDepth');
    // Re-apply injection after numeric fields change
    $('#textme_rp_inject_messages, #textme_rp_inject_days, #textme_rp_inject_depth').on('change', function () {
        updateRpInjection();
    });
    $('#textme_rp_inject_position').val(String(settings.rpInjectPosition ?? 1)).on('change', function () {
        updateSetting('rpInjectPosition', Number($(this).val()));
        context.saveSettingsDebounced();
        updateRpInjection();
    });
    $('#textme_rp_inject_role').val(String(settings.rpInjectRole ?? 0)).on('change', function () {
        updateSetting('rpInjectRole', Number($(this).val()));
        context.saveSettingsDebounced();
        updateRpInjection();
    });
    $('#textme_rp_inject_header').val(settings.rpInjectHeader ?? DEFAULT_RP_HEADER).on('change', function () {
        updateSetting('rpInjectHeader', $(this).val());
        context.saveSettingsDebounced();
        updateRpInjection();
    });
    $('#textme_rp_inject_header_reset').on('click', function () {
        $('#textme_rp_inject_header').val(DEFAULT_RP_HEADER).trigger('change');
        toastr.success('Header reset to default.');
    });

    // ── Schedule ──
    bindCheckbox('#textme_schedule_enabled',          'scheduleEnabled',         'Enable Schedule');
    bindCheckbox('#textme_schedule_include_context',  'scheduleIncludeContext',  'Schedule: Include RP Context');
    bindCheckbox('#textme_schedule_include_wi',       'scheduleIncludeWI',       'Schedule: Include Lorebook');
    ...
    $('#textme_generate_schedule').on('click', async function () {
        ...
        try {
            ...
            btn.prop('disabled', false).html('<i class="fa-solid fa-calendar-plus"></i> Generate Schedule');
        }
    });
    ...
    $('#textme_export_schedule').on('click', function () {
        ...
        if (!json) { toastr.warning('No schedule to export. Generate one first.'); return; }
    ...
    function bindAutonomousThreshold(selector, statusKey) {
        ...
        $(selector).val(val).on('change', function () {
            ...
            settings.autonomousThresholds[statusKey] = newVal;
            context.saveSettingsDebounced();
            log.debug(`[Settings] autonomousThresholds.${statusKey} → ${newVal}min`);
        });
    }
    ...
    // Syncs button label to current level on load and after each click.
    function _refreshLogLevelBtn() {
        const level = getLogLevel();
        const btn   = document.getElementById('textme_toggle_log_level');
        if (!btn) return;
        btn.innerHTML = level === 'debug'
            ? '<i class="fa-solid fa-bug"></i> Log level: DEBUG (click → INFO)'
            : '<i class="fa-solid fa-info-circle"></i> Log level: INFO (click → DEBUG)';
    }
    _refreshLogLevelBtn();

    $('#textme_toggle_log_level').on('click', function () {
        const next = getLogLevel() === 'debug' ? 'info' : 'debug';
        setLogLevel(next);
        _refreshLogLevelBtn();
        log.info(`[Settings] Log level changed → ${next.toUpperCase()}`);
        toastr.info(`TextMe log level: ${next.toUpperCase()}`, '', { timeOut: 1500 });
    });
}
...
/**
 * Guard flag — ensures handleDropdown() is wired up only once per page load.
 *
 * WHY THIS EXISTS:
 * loadSettingsUI() can be called multiple times (e.g. on chat-changed or
 * character-changed events). Each call would previously invoke
 * _initConnectionProfileDropdown() which calls
 * ConnectionManagerRequestService.handleDropdown(). That helper registers a
 * new jQuery .on('change', ...) listener on the element every time
 * it is called — jQuery does NOT deduplicate identical listeners. The result:
 *
 *   - 1st call → 1 onChange handler registered
 *   - 2nd call → 2 handlers (both fire on next change)
 *   - Nth call → N handlers all fire concurrently
 *
 * With N concurrent onChange calls each doing updateSetting() +
 * saveSettingsDebounced(), the ST server receives N simultaneous write
 * requests for the same settings file. On Windows this manifests as a
 * file-locking race that kills the Node.js process — the "Завершить
 * выполнение пакетного файла [Y/N]?" prompt in the console.
 *
 * The fix: skip the entire initialisation on any call after the first.
 * The dropdown value is still restored to the saved profileId by the initial
 * call, so subsequent loadSettingsUI() calls do not need to re-run it.
 */
let _profileDropdownInitialized = false;

/**
 * Initialize the Connection Profile in General settings.
 *
 ...
 *
 * NOTE: No parameters — always reads fresh state via getSettings() /
 * SillyTavern.getContext() to avoid stale closure captures.
 */
async function _initConnectionProfileDropdown() {
    // ── Guard: only wire up the dropdown once per page load ───────────────
    if (_profileDropdownInitialized) {
        log.debug('[Profile] Dropdown already initialized — skipping re-init.');
        return;
    }
    ...
    // Check Connection Manager availability using a fresh context snapshot
    const ctxCheck = SillyTavern.getContext();
    try {
        if (ctxCheck.extensionSettings?.disabledExtensions?.includes('connection-manager')) {
            $select.closest('.textme-profile-row').hide();
            log.debug('[Profile] Connection Manager disabled — profile selector hidden.');
            return;
        }
        const profiles = ctxCheck.extensionSettings?.connectionManager?.profiles;
    ...
    try {
        ...
        // Read the saved profile ID at init time (for the initial selection)
        const initialProfileId = getSettings().connectionProfileId || '';

        ConnectionManagerRequestService.handleDropdown(
            '#textme_connection_profile',
            initialProfileId,
            // onChange — called when user selects a profile OR when the selected profile is deleted.
            // FIX: always call SillyTavern.getContext() fresh here instead of closing over
            // the `context` / `settings` captured during loadSettingsUI(). Those references
            // can become stale if ST reloads its internal state between calls.
            async (profile) => {
                try {
                    const newId   = profile?.id ?? null;
                    const newName = profile?.name ?? '(default)';
                    updateSetting('connectionProfileId', newId);
                    // Fresh context on every invocation — avoids stale-closure crash
                    ...
                    // Prevent unhandled promise rejection from escaping into ST's error boundary
                    log.error('[Profile] onChange handler threw:', err);
                    toastr.error(`TextMe: failed to save connection profile — ${err.message}`);
                }
            },
        );

        // Mark as initialized AFTER handleDropdown() succeeds so a thrown error
        // (e.g. import failure) allows a retry on the next loadSettingsUI() call.
        _profileDropdownInitialized = true;
        log.debug('[Profile] Connection profile dropdown initialized.');
    } catch (e) {
        // shared.js import failed or handleDropdown threw — hide the row gracefully
        $select.closest('.textme-profile-row').hide();
...
// v0.0.3-alpha: autonomousTaskPrompt removed — merged into autonomousPrompt.
// Preset migration (removing the old key from saved presets) is handled in
// state.js mergeDefaults() so it happens on load, not just on UI interaction.
const PRESET_PROMPT_KEYS = ['smsPrompt', 'summaryPrompt', 'schedulePrompt', 'autonomousPrompt'];
...
function _initPromptPresets(settings, context) {
    ...
    $('#textme_preset_delete').on('click', function () {
        ...
        if (!name) { toastr.warning('Select a preset to delete first.'); return; }
