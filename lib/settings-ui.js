...
export function loadSettingsUI() {
    ...
    $('#textme_enabled').prop('checked', settings.enabled).on('change', function () {
        ...
        if (getSettings().enabled) initPhoneUI(); else destroyPhoneUI();
    });
    ...
    try {
        ...
        $('#textme_connection_profile_wrapper').html(
            '<i>Connection Manager not available. Update SillyTavern to staging branch.</i>'
        );
    }
