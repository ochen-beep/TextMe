...
async function assembleSystemPrompt({ task } = {}) {
    ...
    // ── Block 4: <context> ────────────────────────────────────────────────
    // Current time, schedule/status, time-gap indicator, recent RP events.
    try {
        const ctxParts = [];
        ...
        // 4c. Status-aware distraction note (avoid duplicating what getScheduleContext already says)
        const { status: currentStatus } = getCurrentStatus();
        if (currentStatus === 'dnd' || currentStatus === 'idle') {
            ctxParts.push(`[${getCharName()} is currently occupied — short or distracted replies expected, may not address every message.]`);
        }

        // 4d. Time-gap indicator: how long ago did the user last message?
        try {
            const phoneData = getPhoneData();
            ...
            if (msgs?.length > 0) {
                ...
                if (lastUserMsg) {
                    const gapMs      = now.getTime() - lastUserMsg.time;
                    ...
                    // Helper: format gap as human-readable string
                    ...
                    // Pick the most severe applicable level (days > long > short)
                    let tpl = null;
                    ...
                    if (tpl) {
                        // Replace {{gap}} before passing through substituteParams
                        const withGap = tpl.replace(/\{\{gap\}\}/g, fmtGap(gapMinutes));
                        const resolved = sub(withGap);
                        ctxParts.push(resolved);
                        log.debug(`[Prompt] Gap indicator: ${gapMinutes}min → "${resolved.substring(0, 80)}"`);
                    }
                }
            }
        } catch (gapErr) {
            log.debug('[Prompt] Time-gap calculation skipped:', gapErr);
        }
