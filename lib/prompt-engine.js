 ...
 *   - When a Connection Profile ID is configured, TextMe uses
 *     ConnectionManagerRequestService.sendRequest() with its own
 *     AbortController signal — fully independent from ST's main chat.
 *   - When no profile is set, falls back to generateRaw() which shares
 *     ST's global AbortController. In this mode cancellation is best-effort
 *     (flag-based, not signal-based).
 */
...
/**
 * Try to resolve the Connection Profile for TextMe.
 * Returns { profileId, service } or null if not available.
 */
function resolveConnectionProfile() {
    try {
        ...
        if (!service) {
            log.warn('ConnectionManagerRequestService not available in this ST version.');
            return null;
        }

        // Verify the profile exists
        const cm = context.extensionSettings?.connectionManager;
        const profile = cm?.profiles?.find(p => p.id === profileId || p.name === profileId);
        if (!profile) {
            log.warn('Connection profile not found:', profileId);
            return null;
        }

        return { profileId: profile.id, service };
    } catch (e) {
...
/**
 * Extract text content from API response (handles various formats).
 */
...
/**
 * Core generation function used by both phone response and autonomous.
 *
 * @param {string} systemPrompt  Full system prompt.
 * @param {string} conversationText  Conversation prompt.
 * @param {number} maxTokens  Max tokens.
 * @param {AbortSignal} [signal]  Optional AbortSignal (only effective with Connection Profile).
 * @returns {Promise<string>}  Raw response text.
 */
async function callGeneration(systemPrompt, conversationText, maxTokens, signal) {
    const context = SillyTavern.getContext();
    const profile = resolveConnectionProfile();

    if (profile) {
        // ══════════════════════════════════════════════════════════════
        // PATH A: ConnectionManagerRequestService — own AbortController
        // Signal is passed through → cancel button WORKS.
        // ST's #mes_stop does NOT affect this request.
        // ══════════════════════════════════════════════════════════════
        log.info('Using Connection Profile for generation (isolated).');
        ...
        const resp = await profile.service.sendRequest(
            profile.profileId,
            messages,
            maxTokens,
            {
        ...
        // ══════════════════════════════════════════════════════════════
        // PATH B: generateRaw — shares ST's global AbortController.
        // Signal parameter is NOT honoured by generateRaw.
        // Cancel button will NOT work; ST Stop WILL kill this request.
        // ══════════════════════════════════════════════════════════════
        ...
        log.info('Using generateRaw (shared with ST — no cancel isolation).');
...
/**
 * Returns true if the current settings allow true cancel isolation
 * (i.e., we have a Connection Profile configured and the service is available).
 */
export function hasCancelIsolation() {
    return resolveConnectionProfile() !== null;
}
 ...
 *   Only effective when a Connection Profile is configured.
 * @returns {Promise<string[]>}
 */
export async function generatePhoneResponse(signal) {
    ...
    const replyText = (await callGeneration(
        systemPrompt,
        conversationText,
