 ...
 *   an empty response.
 *
 * FIX (v1.4.2): runGeneration() now uses a three-attempt cascade when the
 *   Connection Manager path is used, to handle edge cases where
 *   extractData:true returns empty:
 *     1. extractData:true  — normal path; also handles AsyncGenerator result
 *        (preset may have streaming=true, causing processRequest() to return
 *        a generator instead of a string).
 *     2. extractData:false — manual extraction from raw response object.
 *     3. extractData:false, includePreset:false — strip preset entirely so
 *        it cannot override stream:false.
 *   A descriptive error is thrown if all three attempts fail.
 *
...
/**
 * Extract plain text from a sendRequest() result in any known envelope format.
 *
 * sendRequest() with extractData:true should return a string, but may return
 * null/undefined/object depending on the API type and preset configuration.
 * sendRequest() with extractData:false returns the raw response object.
 *
 * Handles:
 *   - Plain string (extractData:true normal case)
...
function extractTextFromResult(result) {
    ...
    // Anthropic format
    const anthropicText = result?.content?.[0]?.text;
    if (typeof anthropicText === 'string') return anthropicText;
    return '';
}

/**
 * Consume an AsyncGenerator returned when a streaming preset leaks through.
 *
 * When a Connection Profile's preset has streaming=true, processRequest()
 * returns an AsyncGenerator even when stream:false is requested. This helper
...
async function consumeGenerator(gen) {
    ...
    try {
        ...
        log.warn('[Profile] Error consuming stream generator:', e);
    }
    return text;
}
 ...
 *
 * FIX (v1.4.1): For Connection Manager profiles, constructPrompt() is called
 * BEFORE sendRequest(). This converts the messages[] array into an
 * instruct-formatted string for Text Completion profiles (textgenerationwebui),
 * while leaving Chat Completion profiles (openai) unchanged.
 *
 * FIX (v1.4.2): Three-attempt cascade for the Connection Manager path to handle
 * edge cases where extractData:true returns empty:
 *
 *   Attempt 1 — extractData:true, includePreset:true
 *     Normal path. Also handles the case where a streaming-enabled preset
 *     causes processRequest() to return an AsyncGenerator instead of a string.
 *
 *   Attempt 2 — extractData:false, includePreset:true
 *     Receive raw response object, extract text manually from known envelope
 *     formats (OpenAI, text-completion, Anthropic). Also handles AsyncGenerator.
 *
 *   Attempt 3 — extractData:false, includePreset:false, includeInstruct:false
 *     Strip the profile preset entirely to prevent it from overriding
 *     stream:false. Same manual extraction as Attempt 2.
 *
 * If all three attempts return empty, a descriptive error is thrown.
 *
...
async function runGeneration({ systemPrompt, conversationText, maxTokens, signal }) {
    ...
    if (profile) {
        ...
        // constructPrompt() converts messages[] to the correct format:
        //   - Chat Completion profiles (openai)             → messages[] as-is
        ...
        // ── Attempt 1: extractData:true (normal path) ──────────────────────
        try {
            let result = await ConnectionManagerRequestService.sendRequest(
                profile.id,
                prompt,
                maxTokens,
                { stream: false, signal, extractData: true, includePreset: true, includeInstruct: true },
            );
            log.debug(`[Profile] Attempt 1 result type: ${typeof result}`);

            // Handle AsyncGenerator: streaming preset leaked through despite stream:false
            if (result != null && typeof result[Symbol.asyncIterator] === 'function') {
                log.warn('[Profile] Attempt 1: got AsyncGenerator — profile preset likely has streaming=true. Consuming stream...');
                result = await consumeGenerator(result);
            }

            const text = extractTextFromResult(result);
            if (text.trim()) {
                log.debug('[Profile] Attempt 1 succeeded.');
                return text;
            }
            log.warn('[Profile] Attempt 1 returned empty, trying extractData:false...');
        } catch (e) {
            log.warn('[Profile] Attempt 1 (extractData:true) failed:', e.message);
        }

        // ── Attempt 2: extractData:false, manual extraction ────────────────
        try {
            let rawResult = await ConnectionManagerRequestService.sendRequest(
                profile.id,
                prompt,
                maxTokens,
                { stream: false, signal, extractData: false, includePreset: true, includeInstruct: true },
            );
            log.debug('[Profile] Attempt 2 raw result preview:', JSON.stringify(rawResult)?.substring(0, 300));

            // Handle AsyncGenerator
            if (rawResult != null && typeof rawResult[Symbol.asyncIterator] === 'function') {
                log.warn('[Profile] Attempt 2: got AsyncGenerator — consuming stream...');
                rawResult = await consumeGenerator(rawResult);
            }

            const text = extractTextFromResult(rawResult);
            if (text.trim()) {
                log.debug('[Profile] Attempt 2 succeeded.');
                return text;
            }
            log.warn('[Profile] Attempt 2 returned empty, trying without preset...');
        } catch (e) {
            log.warn('[Profile] Attempt 2 (extractData:false) failed:', e.message);
        }

        // ── Attempt 3: no preset — prevents preset overriding stream:false ──
        try {
            let rawResult = await ConnectionManagerRequestService.sendRequest(
                profile.id,
                prompt,
            ...
            log.debug('[Profile] Attempt 3 raw result preview:', JSON.stringify(rawResult)?.substring(0, 300));

            // Handle AsyncGenerator
            if (rawResult != null && typeof rawResult[Symbol.asyncIterator] === 'function') {
                log.warn('[Profile] Attempt 3: got AsyncGenerator — consuming stream...');
                rawResult = await consumeGenerator(rawResult);
            }

            const text = extractTextFromResult(rawResult);
            if (text.trim()) {
                log.debug('[Profile] Attempt 3 succeeded (no preset).');
                return text;
            }
        } catch (e) {
            log.warn('[Profile] Attempt 3 (no preset) failed:', e.message);
        }

        // All attempts exhausted — throw a descriptive error
        throw new Error(
            `Empty response from connection profile "${profile.name}". ` +
            'Possible causes: (1) the profile preset has streaming enabled — disable streaming in the preset; ' +
            '(2) the API endpoint returned an empty response — check your API key and endpoint URL; ' +
            '(3) the profile is misconfigured — verify the Connection Manager profile settings.'
        );
    }
