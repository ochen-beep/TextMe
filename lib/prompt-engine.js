...
import { getScheduleContext, getCurrentStatus } from './schedule.js';
...
async function assembleSystemPrompt({ task } = {}) {
    ...
    // ── Block 1: <character> ──────────────────────────────────────────────
    // Moved first (background/static info). <role> moved last (right before
    // the conversation turn) so its rules have maximum recency weight.
    ...
    try {
        ...
        if (typeof context.getWorldInfoPrompt === 'function') {
            ...
            // This prevents the WI budget from being artificially capped at 70000
            // when the model has a larger context window.
            const modelCtx   = context.max_context ?? 70000;
    ...
    // ── Block 4: <context> ────────────────────────────────────────────────
    // Recent RP events only. Time/status/gap moved to conversation prefix
    // (buildStatusPrefix) so they sit directly above the SMS messages.
    try {
        const chat  = context.chat;
        ...
        if (chat?.length > 0 && limit > 0) {
            ...
            if (recent) {
                blocks.push(block('context', `[Recent RP Events — for context only, do NOT continue the roleplay]\n${recent}`));
    ...
    // ── Block 5: <role> ───────────────────────────────────────────────────
    // Placed LAST in the system prompt (before <task>) so that texting rules
    // and persona framing have maximum recency weight — the model reads them
    // immediately before seeing the conversation turn.
    ...
    try {
        ...
        const modelCtx   = ctx.max_context ?? 70000;
...
// ─────────────────────────────────────────────────────────
// Status prefix (injected at the top of the conversation turn)
// ─────────────────────────────────────────────────────────

/**
 * Build a compact status/time/gap block that goes directly above the SMS
 * messages in the conversation turn. Placing it here means the model sees
 * current context immediately before the dialogue — maximum recency weight.
 *
 * Includes:
 *   - Current time + schedule/status + status modifier (from getScheduleContext)
 *   - Conversation gap indicator (if user has been silent long enough)
 *
 * The duplicate "is currently occupied" note is intentionally removed —
 * getScheduleContext's [Status modifier] already covers this.
 *
 * @returns {string} A block ready to prepend to the conversation text, or ''
 */
function buildStatusPrefix() {
    try {
        const settings = getSettings();
        const context  = SillyTavern.getContext();
        const sub      = context.substituteParams || ((s) => s);
        ...
        parts.push(`Current time: [${timeStr}], ${dayStr}, ${dateStr}.`);

        // Schedule / status + modifier (single source, no duplication)
        const schedCtx = getScheduleContext();
        if (schedCtx?.trim()) parts.push(schedCtx.trim());

        // Conversation gap indicator
        try {
            const phoneData = getPhoneData();
            ...
            if (msgs?.length > 0) {
                ...
                for (let i = msgs.length - 1; i >= 0; i--) {
                    if (msgs[i].isUser && msgs[i].time) { lastUserMsg = msgs[i]; break; }
                }
                if (lastUserMsg) {
                    const gapMinutes = Math.round((now.getTime() - lastUserMsg.time) / 60000);
                    const thresholds = settings.gapThresholds ?? { short: 20, long: 60, days: 1440 };
                    const prompts    = settings.gapPrompts    ?? {};
                    ...
                    let tpl = null;
                    if (thresholds.days > 0 && gapMinutes >= thresholds.days) {
                        tpl = prompts.days ?? '[Conversation gap: {{user}} last messaged ~{{gap}} ago — treat this as restarting the conversation, not continuing it.]';
                    } else if (thresholds.long > 0 && gapMinutes >= thresholds.long) {
                        tpl = prompts.long ?? '[Conversation gap: {{user}} last messaged ~{{gap}} ago — the mood or topic may have shifted.]';
                    } else if (thresholds.short > 0 && gapMinutes >= thresholds.short) {
                        tpl = prompts.short ?? '[Conversation gap: {{user}} last messaged ~{{gap}} ago.]';
                    }

                    if (tpl) {
                        const resolved = sub(tpl.replace(/\{\{gap\}\}/g, fmtGap(gapMinutes)));
                        parts.push(resolved);
                        log.debug(`[Prompt] Status prefix — gap: ${gapMinutes}min → "${resolved.substring(0, 80)}"`);
                    }
                }
            }
        } catch (gapErr) {
            log.debug('[Prompt] Status prefix — gap calc skipped:', gapErr);
        }

        if (parts.length === 0) return '';
        return parts.join('\n') + '\n\n';
    } catch (e) {
        log.warn('[Prompt] buildStatusPrefix failed:', e);
        return '';
    }
}
 ...
 * Prepends a status/time/gap prefix directly above the messages
 * so the model sees current context at maximum recency.
 * @returns {string}
 */
function buildConversationText() {
    ...
    // Status/time/gap prefix — injected right before the conversation
    const statusPrefix = buildStatusPrefix();
...
export async function generateAutonomousMessage(signal) {
    ...
    let taskPrompt = sub(settings.autonomousPrompt || '');

    // Enrich <task> with the character's current activity so the autonomous
    // message is grounded in what they're actually doing right now, not abstract.
    try {
        const { status, activity } = getCurrentStatus();
        if (activity) {
            const charName = getCharName();
            taskPrompt = `[${charName} right now: ${activity} (status: ${status})]\n\n${taskPrompt}`;
            log.debug(`[Autonomous] Current activity injected into <task>: "${activity}" (${status})`);
        }
    } catch (e) {
        log.debug('[Autonomous] Could not inject current activity:', e);
    }
