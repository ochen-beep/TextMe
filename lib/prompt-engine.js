 ...
 *
 * FIX: generateRaw is called with OBJECT signature throughout:
 *   generateRaw({ prompt, system_prompt, max_new_tokens })
 * This is required for Chat Completion APIs (OpenAI/Gemini/Claude).
 * The old positional string-signature routes output through ST's quiet
 * generation pipeline which needs an active chat message slot — when
 * called from an extension context without one, ST throws
 * "No message generated" even though the API returned a valid reply.
 */
 ...
 * FIX: getWorldInfoPrompt is async — must be awaited with trigger text.
...
async function assembleSystemPrompt() {
    ...
    try {
        if (typeof context.getWorldInfoPrompt === 'function') {
            const worldInfo = await context.getWorldInfoPrompt(system, true);
 ...
 *
 * FIX: Object-signature generateRaw({ prompt, system_prompt, max_new_tokens })
 * avoids "No message generated" on Chat Completion APIs.
 *
 * @param {string|null} userMessage
 * @returns {Promise<string[]>}
 */
export async function generatePhoneResponse(userMessage) {
    ...
    // FIX: Object-signature — result returned directly, no chat slot needed.
 ...
 *
 * FIX: Object-signature generateRaw.
 *
 * @returns {Promise<string[]>}
 */
