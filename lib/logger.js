 ...
 *   — Runtime log level: DEBUG messages can be suppressed at runtime without
 *     touching code. Controlled via setLogLevel('info'|'debug').
 *     Default is 'debug' so beta-testers get full verbosity.
 *   — Buffer overflow counter: when MAX_LOG_ENTRIES is hit, the number of
 *     dropped entries is tracked and included in the export header.
 *   — exportLogs() header now includes logLevel and droppedEntries.
 *   — getLogLevel() / setLogLevel() exported for settings-ui.js toggle.
 */
...
/** Count of entries dropped due to buffer overflow since last export/clear. */
let _droppedEntries = 0;

/**
 * Runtime log level.
 * 'debug' — all messages written (default for beta).
 * 'info'  — DEBUG entries are skipped entirely (quieter production mode).
 */
let _logLevel = 'debug';

export function getLogLevel() { return _logLevel; }
...
function addEntry(level, ...args) {
    // Runtime level filter: skip DEBUG when level is 'info'
    if (level === 'DEBUG' && _logLevel === 'info') return;
...
export async function exportLogs() {
    try {
        ...
        const header = [
            ...
            `Dropped (overflow):  ${_droppedEntries}`,
            sep,
